...
def saveMockarooDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + tablename + " (id,first_name,last_name,name,age) \
                   VALUES (? , ? , ? , ? , ? )" 
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
...
def saveFakerDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + fakertable + " (name,address, phone, job, email, company, country, city, text,state) \
                   VALUES (?,?,?,?,?,?,?,?,?,?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
...
def loadDF(targetfile, target):
    if target == 'weather':
        ...
        colsnames = ['name','address', 'phone', 'job', 'email', 'company', 'country',  'city', 'text,state']
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveFakerDf(dfData)
